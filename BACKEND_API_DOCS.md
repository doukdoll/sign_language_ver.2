...
응답의 `price`는 예매와 동일한 DB 스케줄 가격을 사용합니다. 현재 CSV 초기 적재 가격은 30,000원이므로 실제 운임은 아닙니다. `seatType`은 `STANDARD`, `availableSeats`는 50, `status`는 `available`로 고정됩니다. 이 값으로 실제 좌석 재고나 요금을 판단하면 안 됩니다.
...
위 값은 형식 예시입니다. 열차 번호·출발역·도착역·출발 시각·도착 시각이 모두 DB의 동일한 스케줄과 정확히 일치해야 합니다. 검색 응답의 구간 값을 사용하며, 일치하지 않으면 HTTP 400을 반환하고 저장하지 않습니다. 티켓에는 검증한 DB 스케줄의 역과 시각을 사용합니다.

`tripType`을 포함한 문자열 필드는 필수이며 공백만 보낼 수 없습니다. 출발·도착 시각도 필수이고 승객 수는 1~9명입니다. 예약 ID와 QR용 문자열을 생성하여 저장하고, 좌석은 `12호차 34A석`으로 고정합니다. 반환 금액은 검색에 사용한 DB 스케줄 가격 × 승객 수입니다. 실제 결제 승인이나 좌석 재고 차감은 없습니다. 현재 FE 예매 흐름에서도 이 API를 호출하지 않습니다.

### 요청 오류 응답

| 상황 | HTTP 상태 | 응답 |
| --- | --- | --- |
| 필수값 누락·빈 문자열·잘못된 승객 수 | 400 | 필드별 오류 메시지 객체(예: `{"tripType":"Trip type is required"}`) |
| 잘못된 JSON 또는 날짜 형식 | 400 | 문자열 `Invalid request` |
| 예매할 전체 구간과 일치하는 스케줄 없음 | 400 | 문자열 `Invalid train schedule` |
| 예상치 못한 서버 오류 | 500 | 문자열 `An unexpected error occurred` |

예상치 못한 오류의 내부 상세는 서버 로그에만 기록하며 응답에 노출하지 않습니다. 이 표는 Spring HTTP API 기준이며 WebSocket의 구조화된 `ERROR` 메시지는 아래 규약을 따릅니다.
...
현재 BE 테스트 26개는 중계 회귀 12개, 실제 로컬 WebSocket + 모의 AI 통합 1개, 모의 HTTP 서비스 3개, 예매 서비스 2개, 검색 가격 1개, H2 구간 조회 1개, HTTP 입력 검증·오류 응답 6개입니다. [CI](.github/workflows/ci.yml)는 `develop` 대상 PR과 push에서 Java 17·Gradle 8.14.4로 빌드와 테스트를 실행합니다.
