...
> 이 문서는 2025-11-11 로컬 카메라 모드 통합 당시의 기록입니다. 아래 행 번호·예상 로그·성능 수치는 현재 구현이나 재측정 결과를 의미하지 않습니다. 현재 구조와 실행은 [README](README.md), 검증기 적용 범위와 설정 제약은 [최신 가이드](docs/VALIDATOR_INTEGRATION_GUIDE.md), 자동 검증은 [CI 가이드](../docs/CI.md)를 참고하세요. Flask WebSocket 세션 규약 검증과 이 품질 검증기는 별개입니다.
