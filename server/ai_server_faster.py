...
def resample_and_return_deque(
    ...
    """
    deque를 입력받아 NumPy로 보간 처리 후, 결과를 다시 deque로 반환하는 일반 함수.

    Args:
        frame_buffer: shape (original_frames, keypoints, 2)를 가지는 np.ndarray를 원소로 하는 deque
        target_frames: 최종적으로 리샘플링할 프레임 수

    Returns:
        deque: shape (target_frames, keypoints, 2)를 가지는 np.ndarray를 원소로 하는 deque
    """
    # 1. 입력 deque 검사 및 np.ndarray로 변환
    if not frame_buffer:
        # 버퍼가 비어있으면 target_frames 그대로 반환
        return frame_buffer

    # deque의 원소들을 (original_frames, keypoints) 형태의 배열로 변환
    keypoints_sequence = np.array(frame_buffer)

    original_frames = keypoints_sequence.shape[0]

    if original_frames == target_frames:
        # 크기가 같으면 변환 없이 기존 deque의 내용 복사하여 반환
    ...
    # --- 3. 최종 np.ndarray 결과를 다시 deque로 변환 ---
