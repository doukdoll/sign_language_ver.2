...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict, trg_vocab: Vocabulary) -> None:
        ...
        # 매 실행마다 models/ 하위에 타임스탬프 폴더를 생성하여 결과를 분리 저장합니다.
        base_model_dir = self.train_cfg["model_dir"]
        run_suffix = time.strftime("%Y%m%d-%H%M%S")
        final_model_dir = os.path.join(base_model_dir, run_suffix)
        self.model_dir = make_model_dir(final_model_dir, overwrite=False)
