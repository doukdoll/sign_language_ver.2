...
def build_model(
    ...
    if enc_type in ["rnn", "lstm", "gru", "recurrent"]:
    ...
    elif enc_type in ["fixed_length", "fixed_recurrent"]:
        # ONNX 호환 고정 길이 RNN 인코더
        rnn_type = encoder_cfg.get("rnn_type", "GRU")
        encoder_cfg_copy = encoder_cfg.copy()
        encoder_cfg_copy["rnn_type"] = rnn_type
        
        encoder = FixedLengthRecurrentEncoder(
            **{k: v for k, v in encoder_cfg_copy.items() 
               if k not in ['embeddings', 'type']},
            emb_size=recurrent_encoder_emb_size,
        )
    
    elif enc_type in ["attention", "masked_attention"]:
        # ONNX 호환 Attention 기반 RNN 인코더
        rnn_type = encoder_cfg.get("rnn_type", "GRU")
        encoder_cfg_copy = encoder_cfg.copy()
        encoder_cfg_copy["rnn_type"] = rnn_type
        
        encoder = MaskedAttentionEncoder(
            **{k: v for k, v in encoder_cfg_copy.items() 
               if k not in ['embeddings', 'type']},
            emb_size=recurrent_encoder_emb_size,
        )
        ...
        raise ValueError(
            f"알 수 없는 인코더 유형 {enc_type}. "
            f"지원되는 타입: rnn, lstm, gru, recurrent, fixed_length, masked_attention"
        )
