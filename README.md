...
문서 확인 기준: 2026-09-22, 실시간 세션 연동·예약 및 카메라 흐름 정리·HTTP 추론 검증·CI 구성 반영 상태.
...
| 열차 시간표 조회 | 구현 | CSV 기반 H2 시간표를 조회하며 오류·빈 결과를 구분합니다. 검색·예매 가격은 같은 DB 값을 사용합니다. |
| 승객 수·여정 유형·날짜 입력 | UI 구현 | 선택값을 화면 상태로 전달합니다. 고정값만 반환하던 REST 호출은 하지 않습니다. |
...
| 왕복 예매 요약 | UI 구현 | 가는 편·오는 편 열차와 좌석을 보존하고 양편 운임 × 승객 수를 표시합니다. |
...
- 백엔드 Dockerfile은 `gradle bootJar`로 실행 JAR만 생성합니다. 로컬 `clean test bootJar`에서 단일 JAR 생성은 확인했지만 실제 이미지 빌드·기동은 별도 검증이 필요합니다.
...
`develop` 대상 PR과 `develop` push에서 FE·BE·AI 세션 및 HTTP 검사를 실행하도록 [GitHub Actions CI](docs/CI.md)를 구성했습니다. 실제 모델·카메라 검증은 별도입니다.
...
PR #18 당시 [PR CI](https://github.com/doukdoll/sign_language_ver.2/actions/runs/35729782219)와 [develop 병합 후 CI](https://github.com/doukdoll/sign_language_ver.2/actions/runs/35732788235)가 모두 통과했습니다. 당시 테스트 수는 FE 8개, BE 16개, AI 세션 15개입니다. 이후 추가한 예약·카메라·예매·HTTP 테스트의 로컬 결과와 원격 CI 여부는 [최신 검증 기록](docs/CI.md)을 구분해서 확인하세요.

실제 모델을 사용하는 별도 테스트는 [추론 테스트 README](server/tests/INFERENCE_TEST_README.md)를 참고합니다.

## 현재 확인된 제한 사항

- 실시간 인식은 WebSocket v1 세션 규약을 사용합니다. 실제 ONNX 연결은 합성 입력으로 검증했고 카메라 확인은 PR #15 시점의 소유자 확인 기록이 있습니다. 이후 카메라 소유권·종료·재시도 변경의 수동 재검증과 모델 정확도 평가는 별도입니다.
- AI 입력 정책 비교는 원본 수어 영상 확보 대기 상태입니다. 현재 128프레임 수집·180프레임 모델 입력 정책은 유지합니다.
- 승객 수, 편도·왕복, 날짜·시간, 좌석 등급용 REST 인식 API는 여전히 고정 응답이며 현재 FE는 호출하지 않습니다.
- 시간표·가격·좌석 잔여 수는 프로토타입 데이터이며 실제 철도 서비스와 연결되지 않습니다.
