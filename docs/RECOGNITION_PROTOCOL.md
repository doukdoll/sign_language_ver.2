...
상태(2026-09-22 갱신): 실제 FE 세션 코드·BE·Python·ONNX 합성 입력 연결 재검증 완료. PR #15에서 소유자의 카메라 확인 완료를 기록했으며, 이후 카메라 소유권·종료·재시도 변경의 수동 재검증은 별도다. 기존 원격 CI 결과와 추가 회귀 테스트의 로컬 결과는 [CI 기록](CI.md)에서 구분한다. [실행 기록과 재현 방법](LIVE_RECOGNITION_CHECK.md)
